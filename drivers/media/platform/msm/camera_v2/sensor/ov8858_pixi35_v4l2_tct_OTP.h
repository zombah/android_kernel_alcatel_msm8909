...
#ifndef OV8858_PIXI35_CMCC_V4L2_TCT_OTP
#define OV8858_PIXI35_CMCC_V4L2_TCT_OTP
#include "msm_camera_i2c.h"

//Debug log
#define OV8858_PIXI35_CMCC_OTP_DEBUG_ON 0
...
#if OV8858_PIXI35_CMCC_OTP_DEBUG_ON
...
int ov8858_read_otp(struct msm_camera_i2c_client *i2c_client)
{
	int start_addr = 0x7010, end_addr = 0x720F; 
	int i,temp1;
	ov8858_g_client = i2c_client;
	...
	// read OTP into buffer
	OV8858_write_i2c(0x3d84, 0xC0);
	OV8858_write_i2c(0x3d88, 0x70); // OTP start address
	OV8858_write_i2c(0x3d89, 0x10);
	OV8858_write_i2c(0x3d8A, 0x72); // OTP end address
	OV8858_write_i2c(0x3d8B, 0x0a);
	OV8858_write_i2c(0x3d81, 0x01); // load otp into buffer
	
	mdelay(20);

	for (i=start_addr; i<=end_addr; i++) {
		printk("all info[0x%x]=%d\n",i,OV8858_read_i2c(i));		
		OV8858_write_i2c(i, 0x00);
	}
	temp1 = OV8858_read_i2c(0x5002);
	OV8858_write_i2c(0x5002, (0x08 & 0x08) | (temp1 & (~0x08)));	
