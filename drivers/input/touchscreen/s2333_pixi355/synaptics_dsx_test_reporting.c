...
#include "pixi355_Synaptics_TestLimit.h"
...
//add by mike.li for ctp ic info & rawdata show START.
#define MAX_STRING_LEN 256
#define MAX_INT_LEN 33
#define STATUS_BUSY 1
//add by mike.li for ctp ic info & rawdata show END.
...
//add by mike.li for ctp ic info & rawdata show START.
extern struct synaptics_rmi4_data *g_rmi4_data;
static int SetReportType(int value)
{
	char buf[MAX_INT_LEN];
	snprintf(buf, MAX_INT_LEN, "%u", value);
        return test_sysfs_report_type_store (NULL, NULL, buf, strlen(buf) + 1);
}

static int GetReport(int value)
{
	char buf[MAX_INT_LEN];
	snprintf(buf, MAX_INT_LEN, "%u", value);
        return test_sysfs_get_report_store (NULL, NULL, buf, strlen(buf) + 1);
}
				   ...
				   struct device_attribute *attr, char *buf)
{
	...
	int count = 10;
	//bool cartesian = true;
	short int* report_data_16;
	//char*  report_data_8;
	int rx_index;
	int tx_index;

	if (g_rmi4_data->suspend) {
		s += sprintf(s,"TP is in suspend mode, Pls wake up!\n");
		return (s-buf);
	}

	if (!f54)
		return -ENODEV;   // f54 no init
	if (f54->status == STATUS_BUSY)
		return -EAGAIN;
	rx_num = (int)(f54->rx_assigned);
	tx_num = (int)(f54->tx_assigned);
	f54->report_type = (enum f54_report_types)setting;
	//tx_num = f54->rx_assigned;
	//rx_num = f54->tx_assigned;
	...
	switch ( f54->report_type ) {
		...
		default:
			retval = test_do_preparation();
			if (retval < 0) {
				printk("Failed to do preparation,%s,%d\n", __func__, __LINE__);
				return retval;
				}
			break;
		}

	printk("1.mli:%s,Line:%d,ret:%d\n",__func__,__LINE__, retval);
	retval = SetReportType(f54->report_type);
	if (retval < 0) {
		printk("Failed to SetReportType,%s,%d\n", __func__, __LINE__);
		return retval;
	}
	...
	if (retval < 0)
	{
		printk("Failed to GetReport,%s,%d\n", __func__, __LINE__);
		return retval;
	}
	// delay, todo
	msleep(50);
	...
	for (tx_index = 0;tx_index < tx_num; tx_index++)
	{
		for (rx_index = 0;rx_index < rx_num; rx_index++)
		{
			s += sprintf(s, "%d ", *report_data_16);
			report_data_16++;
		}
		s += sprintf(s,"\n");
	}
...
//[START-add by mike.li] limit data for rawdata test.[2015.09.11]
static bool test_ret = true;

static void rawdata_compare_key_data(int data, const int limit_data[])
{
	if (data < limit_data[1] || data > limit_data[2]) {
		printk("MIKE: special data line num: %d, limit_min %d, limit_max %d, out-of range data: %d\n", limit_data[0], limit_data[1], limit_data[2], data);

		test_ret = false;
	}
}

static ssize_t jrd_rd_result_show(struct device *dev,
				   struct device_attribute *attr, char *buf)
{
	char *s =buf;
	int  report_size ;
	...
	for (tx_index = 0;tx_index < tx_num; tx_index++)
	{
		cnt_tmp = tx_index * (rx_num - 1);
		for (rx_index = 0;rx_index < (rx_num - 1); rx_index++)
		{
			if (*report_data_16 < limit_buf[cnt_tmp + rx_index][0] || *report_data_16 > limit_buf[cnt_tmp + rx_index][1])
			{
				printk("MIKE: tx_num: %d rx_num: %d, min %d, max %d, out-of range data: %d\n ", tx_index, rx_index, limit_buf[cnt_tmp + rx_index][0], limit_buf[cnt_tmp + rx_index][1], *report_data_16);
				test_ret = false;
			}
			report_data_16++;
		}

		//compare the key data at the end of column.(line num starts from 0)
		if (rx_index == (rx_num - 1)) {
			switch(tx_index) {
				case 15:
					rawdata_compare_key_data(*report_data_16, limit_line15);
					break;
				case 17:
					rawdata_compare_key_data(*report_data_16, limit_line17);
					break;
				case 19:
					rawdata_compare_key_data(*report_data_16, limit_line19);
					break;
				default:
					break;
			}
...
//struct ctp_ic_info *ctp_ic_info_p;
extern int jrd_get_the_ic_info(void);
extern int jrd_get_the_ic_info1(void);
extern  char * jrd_get_the_ic_info2(char ch[]);
extern u8 tp_fw_version(void);
//struct synaptics_rmi4_data *rmi4_data_test = NULL;

static ssize_t jrd_ctp_info_show(struct device *dev,
					struct device_attribute *attr,char *buf)
{
	int count = 0;
	int device_config_id;
	int device_fw_id;
	unsigned char ic_model_name[16];

	device_config_id = jrd_get_the_ic_info();
	device_fw_id = jrd_get_the_ic_info1();
	jrd_get_the_ic_info2(ic_model_name);
	//show the information
	count+= snprintf(buf + count, PAGE_SIZE - count,  "IC_VENDOR: Synaptics\n");
	count+= snprintf(buf + count, PAGE_SIZE - count,  "MODEL:%s\n", ic_model_name);
	count+= snprintf(buf + count, PAGE_SIZE - count,  "FW_CONFIG_VERSION: %04x\n", device_config_id);
	count+= snprintf(buf + count, PAGE_SIZE - count,  "FW_BUILD_ID: %d\n", device_fw_id);
...
static ssize_t jrd_gesture_switch_show(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "enable_wakeup_gesture = %d\n", g_rmi4_data->enable_wakeup_gesture);
}
...
{
	unsigned int input;
	struct i2c_client* client = to_i2c_client(g_rmi4_data->pdev->dev.parent);;
...
int jrd_ctp_sysfs_init(struct synaptics_rmi4_data *rmi4_data)
{
...
static void synaptics_rmi4_test_remove(struct synaptics_rmi4_data *rmi4_data)
{
	...
	//add by mike.li for ctp ic info & rawdata show.
	jrd_ctp_sysfs_exit();
