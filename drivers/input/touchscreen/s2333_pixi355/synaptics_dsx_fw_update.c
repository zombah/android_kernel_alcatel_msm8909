...
//add by mike.li for ctp ic info START.
char *jrd_get_the_ic_info2(char p [])
{
	int retval;
	...
	retval = synaptics_rmi4_reg_read(rmi4_data,
				...
				ic_model_name,
				10);
	strncpy(p, ic_model_name, 10);

	return p;
}

int jrd_get_the_ic_info1(void)
{
	int device_fw_id;
	struct synaptics_rmi4_data *rmi4_data = fwu->rmi4_data;
...
EXPORT_SYMBOL(jrd_get_the_ic_info);
EXPORT_SYMBOL(jrd_get_the_ic_info1);
EXPORT_SYMBOL(jrd_get_the_ic_info2);
//add by mike.li for ctp ic info END.
