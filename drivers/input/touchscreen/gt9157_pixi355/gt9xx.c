...
static int jrd_gesture_status = 0;

static int jrd_ctp_sysfs_init(struct goodix_ts_data *ts);
...
//Add by Luoxingxing-START
#if JRD_HAD_WIN_COVER
int jrd_cover_status = 0;
/*Add by Luoxingxing for window cover*/
u8 cover_config[GTP_CONFIG_MAX_LENGTH + GTP_ADDR_LENGTH]
                = {GTP_REG_CONFIG_DATA >> 8, GTP_REG_CONFIG_DATA & 0xff};
static s32 gtp_init_panel_with_cover(struct goodix_ts_data *ts);
s32 gtp_send_cfg_with_cover(struct i2c_client *client);
#endif
//Add by Luoxingxing-END
...
extern u16 uniformity_lmt;       // screen uniformity in percent
...
extern void gtp_open_test_init(struct i2c_client *client);
				   ...
				   struct device_attribute *attr, char *buf)
{
	int count = 0;
    	...
    	count +=snprintf(buf + count, PAGE_SIZE - count,"%s\n", 
             jrd_gesture_status? "Gesture_enable" : "Gesture_disable");
				   ...
				   struct device_attribute *attr, char *buf)
{
	...
	int ret = 0;
	u8 opr_buf[2] = {0};
	u8 v_buf[8] = {GTP_REG_VERSION >> 8, GTP_REG_VERSION & 0xff};

	if(sys_ts_data->gtp_is_suspend == 1||gtp_resetting == 1)
	{
		count +=snprintf(buf + count, PAGE_SIZE - count,"NOTICE:TP was suspend or resetting!\n");
	}

	//1. Get the Chip type.
//	count +=snprintf(buf + count, PAGE_SIZE - count,"Chip_Vendor: %s\n", 
//			(sys_ts_data->chip_type == CHIP_TYPE_GT9)? "GOODIX_GT9" : "GOODIX_GT9F");

	count +=snprintf(buf + count, PAGE_SIZE - count,"Chip_Vendor:GOODIX_GT9\n");
	//2. Read IC version
	ret = gtp_i2c_read(sys_ts_data->client, v_buf, sizeof(v_buf));
	if (ret < 0)
	{
		count +=snprintf(buf + count, PAGE_SIZE - count,"GTP read IC version failed\n");
	}
	
	//3. Read config version	 
	ret = gtp_i2c_read_dbl_check(sys_ts_data->client, GTP_REG_CONFIG_DATA, &opr_buf[0], 1);
	if (ret < 0)
	{
	    count +=snprintf(buf + count, PAGE_SIZE - count,"Read Config version failed!\n");
	}

	if (v_buf[5] == 0x00)
	{
		count +=snprintf(buf + count, PAGE_SIZE - count,
			"IC_Version: %c%c%c_%02x%02x\n", v_buf[2], v_buf[3], v_buf[4], v_buf[7], v_buf[6]);
	}else{
		count +=snprintf(buf + count, PAGE_SIZE - count,
			"IC_Version: %c%c%c%c_%02x%02x\n", v_buf[2], v_buf[3], v_buf[4], v_buf[5], v_buf[7], v_buf[6]);
	}
	
    	count +=snprintf(buf + count, PAGE_SIZE - count,"Config_Version:0x%02X\n", opr_buf[0]);
		
	return count;
}

static s32 rawdata_cnt = 1;
#define MAX_X_NODE_CNT	12
#define MAX_Y_NODE_CNT	22
#define MAX_AA_NODE		MAX_X_NODE_CNT*(MAX_Y_NODE_CNT -1)
#define MAX_ERR_RWDATA_COUNT		3

u16 *rawdata_buf = NULL;
static int  gtp_get_rawdata_value(struct i2c_client* client)
{
	int ret = 0;
...
//	ssize_t num_read_chars = 0;
	...
	if (ret == FAIL)
	{
		s += sprintf(s, "%s", "Parse config failed!\n");
	}

	gt9_read_raw_cmd(i2c_connect_client);
	for (i = 0; i < rawdata_cnt; ++i)
	{
		if (gtp_get_rawdata_value(client) <= 0){
			s += sprintf(s, "%s", "could not get rawdata\n");
		}
		else
		{
			for(j = 0; j < MAX_AA_NODE; j++)
			{
				s += sprintf(s, "%u ", rawdata_buf[j]);
				if(!((j+1)%12))
				{
					s += sprintf(s, "\n");
				}
			...
			s += sprintf(s, "%u %u %u \n", 
						rawdata_buf[253], rawdata_buf[257],rawdata_buf[261]);
			s += sprintf(s, "\n");
		}
	}
	//add link 2014-3-28
				   ...
				   struct device_attribute *attr, char *buf)
{
	...
	u16 max_value = 0;
	u16 min_value = 0;
	char *key_name[] = {"Back key", "Home key", "Menu key"};
	...
	//AA Zone
	min_value = rawdata_buf[0];
	for (i = 0; i < MAX_AA_NODE; i ++){
		if((rawdata_buf[i] > max_limit_value || rawdata_buf[i] < min_limit_value) 
				&& (j < MAX_ERR_RWDATA_COUNT )){
			if (j == 0){
				s += sprintf(s,"NG\n");
			}
			s += sprintf(s,"X%dY%d(%u)\n", (i%12) +1,(i/12) +1, rawdata_buf[i]);
			j++;
		}
		max_value = rawdata_buf[i] > max_value ? rawdata_buf[i] : max_value;
		min_value = rawdata_buf[i] < min_value ? rawdata_buf[i] : min_value;	
	}
	if (j == 0){
		s += sprintf(s,"PASS\n");
	}
	...
	s += sprintf(s,"**************************\nKey Zone:");
	j = 0;
	if (rawdata_buf[253] > max_limit_key ||rawdata_buf[253]  < min_limit_key){
		if (j == 0){
			s += sprintf(s,"NG\n");
		}
		s += sprintf(s,"%s: %u\n", key_name[0],rawdata_buf[253] );	
		j = 1;
	}
	if (rawdata_buf[257] > max_limit_key ||rawdata_buf[257]  < min_limit_key){
		if (j == 0){
			s += sprintf(s,"NG\n");
		}
		s += sprintf(s,"%s: %u\n", key_name[1],rawdata_buf[257] );	
		j =1;
	}		
	if (rawdata_buf[261] > max_limit_key ||rawdata_buf[261]  < min_limit_key){
		if (j == 0){
			s += sprintf(s,"NG\n");
		}
		s += sprintf(s,"%s: %u\n", key_name[2],rawdata_buf[261] );	
		j = 1;
	}
				   ...
				   struct device_attribute *attr, char *buf)
{
	int count = 0;
    	...
    	count +=snprintf(buf + count, PAGE_SIZE - count,"%s\n", 
             jrd_cover_status? "Cover_closed." : "Cover_leaved.");
				    ...
				    const char *buf, size_t size)
{
	...
	jrd_cover_status= data ? 1 : 0;
	
	GTP_INFO("===%s===", jrd_cover_status? "Cover_closed." : "Cover_leaved.");
	
	mutex_lock(&gt_cfg_lock);
	if (jrd_cover_status == 1){
		ret = gtp_send_cfg_with_cover(client);
		if (ret < 0){
			GTP_ERROR("gtp_send_cfg_with_cover failed!");
		}
	}else{
        	gtp_send_cfg(client);
	}
 	msleep(300);
	mutex_unlock(&gt_cfg_lock);
	return size;
}
#endif

extern int jrd_gt9xx_short_test(struct i2c_client * client);
extern int jrd_gt9xx_open_test(struct i2c_client * client);

static ssize_t jrd_short_test_show(struct device *dev,
				   struct device_attribute *attr, char *buf)
{
	int count = 0;
	int ret = 0;
	...
	ret = jrd_gt9xx_short_test(client);
			
    	count +=snprintf(buf + count, PAGE_SIZE - count,"short_test:%s\n", 
             ret? "PASS." : "FAIL.");		
	
	return count;
}

static ssize_t jrd_short_test_store(struct device *dev,
				    struct device_attribute *attr,
				    const char *buf, size_t size)
{
	return size;
...
static ssize_t jrd_open_test_show(struct device *dev,
				   struct device_attribute *attr, char *buf)
{
	int count = 0;
	int ret = 0;
	...
	ret = jrd_gt9xx_open_test(client);
			
    	count +=snprintf(buf + count, PAGE_SIZE - count,"open_test:%s\n", 
             ret? "PASS." : "FAIL.");			
	return count;
}

static ssize_t jrd_open_test_store(struct device *dev,
				    struct device_attribute *attr,
				    const char *buf, size_t size)
{
	return size;
...
static DEVICE_ATTR(jrd_gesture_switch, 0664,  
					jrd_gesture_switch_show, jrd_gesture_switch_store);  
static DEVICE_ATTR(jrd_tp_ic_info, 0644,  
					jrd_tp_ic_info_show, NULL);
static DEVICE_ATTR(rawdata, 0644,  
					jrd_read_rawdata_show, jrd_read_rawdata_store); 
static DEVICE_ATTR(rd_result, 0644,
					jrd_rawdata_result_show, jrd_rawdata_result_store); 

#if JRD_HAD_WIN_COVER
static DEVICE_ATTR(jrd_cover_status, 0664,
					jrd_cover_status_show, jrd_cover_status_store); 
#endif

static DEVICE_ATTR(tp_short_test, 0644,
					jrd_short_test_show, jrd_short_test_store); 
static DEVICE_ATTR(tp_open_test, 0644,
					jrd_open_test_show, jrd_open_test_store); 
...
#endif	
	&dev_attr_tp_short_test.attr,
	&dev_attr_tp_open_test.attr,
	NULL,
};
...
static int jrd_ctp_sysfs_init(struct goodix_ts_data *ts)
{ 
	GTP_INFO("Start to Create JRD_TP_sysfs_init.");
	
#if JRD_HAD_WIN_COVER	
	mutex_init(&gt_cfg_lock);
#endif
...
static s32 gtp_init_panel_with_cover(struct goodix_ts_data *ts)
{
	...
	sensor_id = 1;		//Set sensor_id = 1 to select cfg-group1.
	ret = gtp_parse_dt_cfg(&ts->client->dev, &cover_config[GTP_ADDR_LENGTH], &ts->gtp_cfg_len, sensor_id);
		if (ret < 0) {
		GTP_ERROR("Failed to parse config data form device tree.");
		ts->pnl_init_error = 1;
		return -1;
	...
	if (ret == SUCCESS) {
		GTP_DEBUG("Config Version: %d, 0x%02X; IC Config Version: %d, 0x%02X",
		        cover_config[GTP_ADDR_LENGTH], cover_config[GTP_ADDR_LENGTH], opr_buf[0], opr_buf[0]);

		flash_cfg_version = opr_buf[0];
		drv_cfg_version = cover_config[GTP_ADDR_LENGTH];

		if (flash_cfg_version < 90 && flash_cfg_version > drv_cfg_version) {
			cover_config[GTP_ADDR_LENGTH] = 0x00;
		}
	} else {
		GTP_ERROR("Failed to get ic config version!No config sent!");
		return -1;
...
#if GTP_CUSTOM_CFG
	cover_config[RESOLUTION_LOC]     = (u8)GTP_MAX_WIDTH;
	cover_config[RESOLUTION_LOC + 1] = (u8)(GTP_MAX_WIDTH>>8);
	cover_config[RESOLUTION_LOC + 2] = (u8)GTP_MAX_HEIGHT;
	cover_config[RESOLUTION_LOC + 3] = (u8)(GTP_MAX_HEIGHT>>8);

	if (GTP_INT_TRIGGER == 0)  //RISING
	{
		cover_config[TRIGGER_LOC] &= 0xfe; 
	}
	else if (GTP_INT_TRIGGER == 1)  //FALLING
	{
		cover_config[TRIGGER_LOC] |= 0x01;
	}
#endif  // GTP_CUSTOM_CFG
	...
	for (i = GTP_ADDR_LENGTH; i < ts->gtp_cfg_len; i++)
	{
		check_sum += cover_config[i];
	}
	cover_config[ts->gtp_cfg_len] = (~check_sum) + 1;
...
s32 gtp_send_cfg_with_cover(struct i2c_client *client)
{
    ...
    for (retry = 0; retry < 5; retry++)
    {
        ret = gtp_i2c_write(client, cover_config , GTP_CONFIG_MAX_LENGTH + GTP_ADDR_LENGTH);
        if (ret > 0)
        {
            break;
        }
...
static int gtp_power_switch(struct i2c_client *client, int on)
{
	...
	if (!vdd_ana) {
		...
		if (IS_ERR(vdd_ana)) {
			GTP_ERROR("regulator get_of vdd_ana failed");
			ret = PTR_ERR(vdd_ana);
			vdd_ana = NULL;
			return ret;
		}
...
/*jerry [BUGFIX]TP INT pull-disable.*/
    ...
    if (ret < 0)
    {
		//add by mike.li for be compatible with 2nd src.[PR1011143]
        GTP_ERROR("MIKE: I2C communication ERROR!");
		GTP_GPIO_FREE(gtp_rst_gpio);
        GTP_GPIO_FREE(gtp_int_gpio);
...
#if JRD_HAD_WIN_COVER
	ret = gtp_init_panel_with_cover(ts);
	if (ret < 0)
	{
		GTP_ERROR("gtp_init_panel_with_cover failed.");
	}
#endif	
...
/*jerry [BUGFIX]TP INT pull-disable.*/
	...
	if (jrd_cover_status == 1){
		ret = gtp_send_cfg_with_cover(ts->client);
		if (ret < 0){
			GTP_ERROR("gtp_send_cfg_with_cover failed!");
		}
	}else
