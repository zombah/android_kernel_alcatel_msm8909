...
static int vps_set_enable(struct sensors_classdev *sensors_cdev,unsigned int enable)
{
	...
	if ( 0 == enable )
		{
		if(enable_irq_wake_flag == 1)
		{
			if (device_may_wakeup(&gtp_912_i2c_connect_client->dev))
			{
				ret=disable_irq_wake(ts->client->irq);
				enable_irq_wake_flag = 0;
				if(ret)
				GTP_ERROR("vps_set_enable===disable_irq_wake  fail==ret=%d==enable=%d==\n",ret,enable);
			}
			else
				GTP_ERROR("vps_set_enable====device may not wakeup==enable=%d=\n",enable);
		}
		device_init_wakeup(&gtp_912_i2c_connect_client->dev, 0);
		input_report_abs(vps->virtualdevice, ABS_DISTANCE, 1);
		input_sync(vps->virtualdevice);
		wake_lock_timeout(&gt_wakelock, 1*HZ);
		}
	else if ( 1 == enable )
		{
		device_init_wakeup(&gtp_912_i2c_connect_client->dev, 1);
		}
	else
		{
		GTP_ERROR("vps_set_enable = %d \n",enable);
		}
...
//Add begin by TCTNB-QW	2015/04/23		FR978057
	...
	if(vps->vps_enabled)
	{
		if(finger&(1<<6))
		{
			GTP_INFO("distance cat\n");
			input_report_abs(vps->virtualdevice, ABS_DISTANCE, 0);
			input_sync(vps->virtualdevice);
		}
		else if(0==(finger&(1<<6)))
		{
			GTP_INFO("distance not cat \n");
			wake_lock_timeout(&gt_wakelock, 1*HZ);
			input_report_abs(vps->virtualdevice, ABS_DISTANCE, 1);
			input_sync(vps->virtualdevice);
		}
...
static s8 gtp_wakeup_sleep(struct goodix_ts_data * ts)
{
    ...
    if (CHIP_TYPE_GT9F == ts->chip_type)
    {
        ...
        msleep(50);//Mod by TCT-NB.qw for PR 1015447
...
//Add begin by TCTNB-QW  FR978057  2015/04/23
	...
	wake_lock_init(&gt_wakelock,WAKE_LOCK_SUSPEND, "gt912");
	if(ret< 0)
	{
		pr_err("[***%s: failed\n", __func__);
		return ret;
...
static ssize_t firm_ver_store(struct device *dev,
...
static DEVICE_ATTR(firm_ver, 0664, firm_ver_show, firm_ver_store);
...
u8 gup_get_fw_version(u16 *fw_info,u8 *module)
{
	...
	memcpy(&fw_head, &gtp_default_FW_fl[0], FW_HEAD_LENGTH);
...
if(vps->vps_enabled)
	{
	if(enable_irq_wake_flag == 0)
		{
		/*add for virtual Psensor can't wake up lcd when make call*/
		if (device_may_wakeup(&gtp_912_i2c_connect_client->dev))
		{
			err=enable_irq_wake(ts->client->irq);
			enable_irq_wake_flag = 1;
			if(err)
			GTP_ERROR("goodix_ts_suspend===enable_irq_wake  fail==err=%d=\n",err);
		}
		else
		GTP_ERROR("goodix_ts_suspend====device may not wakeup====\n");
		return 0;
		}
	}
#endif
...
if(vps->vps_enabled)
	...
	if(enable_irq_wake_flag == 1)
	{
		/*add for virtual Psensor can't wake up lcd when make call*/
			if (device_may_wakeup(&gtp_912_i2c_connect_client->dev))
			{
				err=disable_irq_wake(ts->client->irq);
				enable_irq_wake_flag = 0;
				if(err)
					GTP_ERROR("goodix_ts_resume===disable_irq_wake  fail==err=%d=\n",err);
			}
			else
			GTP_ERROR("goodix_ts_resume====device may not wakeup====\n");
			ts->gtp_is_suspend = 0;
			return 0;
		}
}
...
/* [PREFORMANCE]-Add-BEGIN by TCTNB.QW, PR-984902, 2015/06/25 decrease resume screen time */
...
/* [PERFORMANCE]-Add-END by TCTNB.QW, PR-984902, 2015/06/25 decrease resume screen time */
