...
extern s32 gup_912_enter_update_mode(struct i2c_client *client);
extern void gup_912_leave_update_mode(void);
extern s32 gup_912_update_proc(void *dir);
#endif

extern void gtp_912_irq_disable(struct goodix_ts_data *);
extern void gtp_912_irq_enable(struct goodix_ts_data *);
...
#endif
s32 GT_912_DATA_LENGTH = 0;
s8 GT912_IC_TYPE[16] = "GT9XX";
...
{
//    if (!strncmp(GT912_IC_TYPE, "GT818", 5) || !strncmp(GT912_IC_TYPE, "GT816", 5)
//        || !strncmp(GT912_IC_TYPE, "GT811", 5) || !strncmp(GT912_IC_TYPE, "GT818F", 6)
//        || !strncmp(GT912_IC_TYPE, "GT827", 5) || !strncmp(GT912_IC_TYPE,"GT828", 5)
//        || !strncmp(GT912_IC_TYPE, "GT813", 5))
    if (strncmp(GT912_IC_TYPE, "GT8110", 6) && strncmp(GT912_IC_TYPE, "GT8105", 6)
        && strncmp(GT912_IC_TYPE, "GT801", 5) && strncmp(GT912_IC_TYPE, "GT800", 5)
        && strncmp(GT912_IC_TYPE, "GT801PLUS", 9) && strncmp(GT912_IC_TYPE, "GT811", 5)
        && strncmp(GT912_IC_TYPE, "GTxxx", 5) && strncmp(GT912_IC_TYPE, "GT9XX", 5))
    {
...
s32 gt912_init_wr_node(struct i2c_client *client)
{
    s32 i;
    ...
    if (i)
    {
        GT_912_DATA_LENGTH = i * DATA_LENGTH_UINT + GTP_ADDR_LENGTH;
        GTP_INFO("Applied memory size:%d.", GT_912_DATA_LENGTH);
    }
    else
    {
 ...
 void gt912_uninit_wr_node(void)
{
    kfree(cmd_head.data);
    cmd_head.data = NULL;
    unregister_i2c_func();
...
ssize_t goodix_tool_write(struct file *filp, const char __user *buff, size_t len, loff_t *off)
{
    ...
    else if (3 == cmd_head.wr)  //Write ic type
    {
        ...
        memcpy(GT912_IC_TYPE, cmd_head.data, cmd_head.data_len);
    ...
    {
        gtp_912_irq_disable(i2c_get_clientdata(gt_client));

    #if GTP_ESD_PROTECT
        gtp_912_esd_switch(gt_client, SWITCH_OFF);
    #endif
    }
    else if (9 == cmd_head.wr) //enable irq!
    {
        gtp_912_irq_enable(i2c_get_clientdata(gt_client));

    #if GTP_ESD_PROTECT
        gtp_912_esd_switch(gt_client, SWITCH_ON);
    #endif
    }
    ...
    {
        if (FAIL == gup_912_enter_update_mode(gt_client))
        {
            return -EPERM;
        }
    }
    ...
    else if (15 == cmd_head.wr) //Update firmware!
    {
        ...
        if (FAIL == gup_912_update_proc((void*)cmd_head.data))
        {
            return -EPERM;
        }
    }
...
ssize_t goodix_tool_read(struct file *file, char __user *page, size_t size, loff_t *ppos)
{
    ...
    else if (!cmd_head.wr)
    {
        ...
        while(data_len > 0)
        {
            if (data_len > GT_912_DATA_LENGTH)
            {
                len = GT_912_DATA_LENGTH;
            }
            else
            {
                len = data_len;
    ...
    {
        ret = simple_read_from_buffer(page, size, ppos, GT912_IC_TYPE, sizeof(GT912_IC_TYPE));
        return ret;
    }
    else if (4 == cmd_head.wr)
    {
        u8 progress_buf[4];
        progress_buf[0] = gt912_show_len >> 8;
        progress_buf[1] = gt912_show_len & 0xff;
        progress_buf[2] = gt912_total_len >> 8;
        progress_buf[3] = gt912_total_len & 0xff;
