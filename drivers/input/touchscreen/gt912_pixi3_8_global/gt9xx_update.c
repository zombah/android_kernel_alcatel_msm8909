...
st_update_msg gt912_update_msg;
u16 gt912_show_len;
u16 gt912_total_len;
static u8 got_file_flag = 0;
static u8 searching_file = 0;

extern u8 gt912_config[GTP_CONFIG_MAX_LENGTH + GTP_ADDR_LENGTH];
extern void gtp_912_reset_guitar(struct i2c_client *client, s32 ms);
extern s32  gtp_912_send_cfg(struct i2c_client *client);
extern s32 gtp_912_read_version(struct i2c_client *, u16* );
extern struct i2c_client * gtp_912_i2c_connect_client;
extern void gtp_912_irq_enable(struct goodix_ts_data *ts);
extern void gtp_912_irq_disable(struct goodix_ts_data *ts);
extern s32 gtp_912_i2c_read_dbl_check(struct i2c_client *, u16, u8 *, int);
...
#if GTP_ESD_PROTECT
extern void gtp_912_esd_switch(struct i2c_client *, s32);
#endif
...
static s32 gup_init_panel(struct goodix_ts_data *ts)
{
    ...
    if (ret == SUCCESS)
    {
        GTP_DEBUG("CFG_GROUP%d Config Version: %d, IC Config Version: %d", sensor_id,
                    gt912_config[GTP_ADDR_LENGTH], opr_buf[0]);
    ...
    if (ret < 0)
    {
        GTP_ERROR("Send gt912_config[ error.");
    }
    ...
    gtp_912_read_version(ts->client, &version);
    msleep(10);
    return 0;
}
...
static u8 gup_get_ic_fw_msg(struct i2c_client *client)
{
    ...
    ret = gtp_912_i2c_read_dbl_check(client, GUP_REG_HW_INFO, &buf[GTP_ADDR_LENGTH], 4);
    if (FAIL == ret)
    {
    ...
    for(i=0; i<4; i++)
    {
        gt912_update_msg.ic_fw_msg.hw_info[i] = buf[GTP_ADDR_LENGTH + 3 - i];
    }
    GTP_DEBUG("IC Hardware info:%02x%02x%02x%02x", gt912_update_msg.ic_fw_msg.hw_info[0], gt912_update_msg.ic_fw_msg.hw_info[1],
                                                   gt912_update_msg.ic_fw_msg.hw_info[2], gt912_update_msg.ic_fw_msg.hw_info[3]);
    ...
    for(retry=0; retry<2; retry++)
    {
        ...
        gt912_update_msg.force_update = buf[GTP_ADDR_LENGTH];
        if((0xBE != gt912_update_msg.force_update)&&(!retry))
        {
    ...
    GTP_DEBUG("IC force update flag:0x%x", gt912_update_msg.force_update);

    // step3:get pid & vid
    ret = gtp_912_i2c_read_dbl_check(client, GUP_REG_PID_VID, &buf[GTP_ADDR_LENGTH], 6);
    if (FAIL == ret)
    {
    ...
    memset(gt912_update_msg.ic_fw_msg.pid, 0, sizeof(gt912_update_msg.ic_fw_msg.pid));
    memcpy(gt912_update_msg.ic_fw_msg.pid, &buf[GTP_ADDR_LENGTH], 4);
    GTP_DEBUG("IC Product id:%s", gt912_update_msg.ic_fw_msg.pid);
    ...
    /*|-----FLASH-----RAM-----|
      ...
      |------9110-----9110----|
      |------9110P----9111----|*/
    if(gt912_update_msg.ic_fw_msg.pid[0] != 0)
    {
        if(!memcmp(gt912_update_msg.ic_fw_msg.pid, "9111", 4))
        {
            GTP_DEBUG("IC Mapping Product id:%s", gt912_update_msg.ic_fw_msg.pid);
            memcpy(gt912_update_msg.ic_fw_msg.pid, "9110P", 5);
        }
    }

    gt912_update_msg.ic_fw_msg.vid = buf[GTP_ADDR_LENGTH+4] + (buf[GTP_ADDR_LENGTH+5]<<8);
    GTP_DEBUG("IC version id:%04x", gt912_update_msg.ic_fw_msg.vid);

    return SUCCESS;
}

s32 gup_912_enter_update_mode(struct i2c_client *client)
{
    s32 ret = -1;
    s32 retry = 0;
    u8 rd_buf[3];
...
void gup_912_leave_update_mode(void)
{
    GTP_GPIO_AS_INT(gtp_int_gpio);

    GTP_DEBUG("[leave_update_mode]reset chip.");
    gtp_912_reset_guitar(gtp_912_i2c_connect_client, 20);
}
...
static u8 gup_enter_update_judge(st_fw_head *fw_head)
{
    ...
    GTP_INFO("IC HARDWARE INFO:%02x%02x%02x%02x", gt912_update_msg.ic_fw_msg.hw_info[0], gt912_update_msg.ic_fw_msg.hw_info[1],
             gt912_update_msg.ic_fw_msg.hw_info[2], gt912_update_msg.ic_fw_msg.hw_info[3]);
    GTP_INFO("IC PID:%s", gt912_update_msg.ic_fw_msg.pid);
    GTP_INFO("IC VID:%04x", gt912_update_msg.ic_fw_msg.vid);

    if (!memcmp(fw_head->pid, "9158", 4) && !memcmp(gt912_update_msg.ic_fw_msg.pid, "915S", 4))
    {
        GTP_INFO("Update GT915S to GT9158 directly!");
        return SUCCESS;
    }
    //First two conditions
    if (!memcmp(fw_head->hw_info, gt912_update_msg.ic_fw_msg.hw_info, sizeof(gt912_update_msg.ic_fw_msg.hw_info)))
    {
        fw_len = 42 * 1024;
    }
    else
    ...
    if (gt912_update_msg.fw_total_len != fw_len)
    {
        GTP_ERROR("Inconsistent firmware size, Update aborted! Default size: %d(%dK), actual size: %d(%dK)", fw_len, fw_len/1024, gt912_update_msg.fw_total_len, gt912_update_msg.fw_total_len/1024);
        return FAIL;
    }
    GTP_INFO("Firmware length:%d(%dK)", gt912_update_msg.fw_total_len, gt912_update_msg.fw_total_len/1024);

    if (gt912_update_msg.force_update != 0xBE)
    {
    ...
    // 20130523 start
    if (strlen(gt912_update_msg.ic_fw_msg.pid) < 3)
    {
    ...
    else
    {
        for (i = 0; i < 3; i++)
        {
            if ((gt912_update_msg.ic_fw_msg.pid[i] < 0x30) || (gt912_update_msg.ic_fw_msg.pid[i] > 0x39))
            {
    ...
    if (pid_cmp_len < strlen(gt912_update_msg.ic_fw_msg.pid))
    {
        pid_cmp_len = strlen(gt912_update_msg.ic_fw_msg.pid);
    }

    if ((!memcmp(fw_head->pid, gt912_update_msg.ic_fw_msg.pid, pid_cmp_len)) ||
            (!memcmp(gt912_update_msg.ic_fw_msg.pid, "91XX", 4))||
            (!memcmp(fw_head->pid, "91XX", 4)))
    {
        ...
        //The third condition
        if (fw_head->vid > gt912_update_msg.ic_fw_msg.vid)
        {
...
static s8 gup_update_config(struct i2c_client *client)
{
    ...
    if(NULL == gt912_update_msg.cfg_file)
    {
        GTP_ERROR("[update_cfg]No need to upgrade config!");
        return FAIL;
    }
    file_len = gt912_update_msg.cfg_file->f_op->llseek(gt912_update_msg.cfg_file, 0, SEEK_END);
    ...
    gt912_update_msg.cfg_file->f_op->llseek(gt912_update_msg.cfg_file, 0, SEEK_SET);

    GTP_DEBUG("[update_cfg]Read config from file.");
    ret = gt912_update_msg.cfg_file->f_op->read(gt912_update_msg.cfg_file, (char*)pre_buf, file_len, &gt912_update_msg.cfg_file->f_pos);
    if(ret<0)
    {
        ...
        if ( (search_type & AUTO_SEARCH_CFG) && !(got_file_flag & CFG_FILE_READY) )
        {
            ...
            if (IS_ERR(pfile))
            {
                ...
                if (!IS_ERR(pfile))
                {
                    ...
                    gt912_update_msg.cfg_file = pfile;
                }
            }
            else
            {
...
static u8 gup_check_update_file(struct i2c_client *client, st_fw_head* fw_head, u8* path)
{
    ...
    if (path)
    {
        ...
        gt912_update_msg.file = filp_open(path, O_RDONLY, 0);

        if (IS_ERR(gt912_update_msg.file))
        {
    ...
    else
    {
        ...
        gt912_update_msg.fw_total_len = sizeof(gtp_default_FW) - FW_HEAD_LENGTH;
        ...
        GTP_DEBUG("Firmware actual size: %d(%dK)", gt912_update_msg.fw_total_len, gt912_update_msg.fw_total_len/1024);
        memcpy(fw_head, &gtp_default_FW[0], FW_HEAD_LENGTH);

        //check firmware legality
        fw_checksum = 0;
        for(i=0; i< gt912_update_msg.fw_total_len; i+=2)
        {
        ...
        if (got_file_flag & CFG_FILE_READY)
        {
            ret = gup_update_config(gtp_912_i2c_connect_client);
            if(ret <= 0)
            {
                GTP_ERROR("Update config failed.");
            }
            _CLOSE_FILE(gt912_update_msg.cfg_file);
    ...
    gt912_update_msg.old_fs = get_fs();
    set_fs(KERNEL_DS);

    gt912_update_msg.file->f_op->llseek(gt912_update_msg.file, 0, SEEK_SET);
    gt912_update_msg.fw_total_len = gt912_update_msg.file->f_op->llseek(gt912_update_msg.file, 0, SEEK_END);
    if (gt912_update_msg.fw_total_len < (FW_HEAD_LENGTH + FW_SECTION_LENGTH*4+FW_DSP_ISP_LENGTH+FW_DSP_LENGTH+FW_BOOT_LENGTH))
    {
        GTP_ERROR("INVALID bin file(size: %d), update aborted.", gt912_update_msg.fw_total_len);
        return FAIL;
    }

    gt912_update_msg.fw_total_len -= FW_HEAD_LENGTH;

    GTP_DEBUG("Bin firmware actual size: %d(%dK)", gt912_update_msg.fw_total_len, gt912_update_msg.fw_total_len/1024);

    gt912_update_msg.file->f_op->llseek(gt912_update_msg.file, 0, SEEK_SET);
    ret = gt912_update_msg.file->f_op->read(gt912_update_msg.file, (char*)buf, FW_HEAD_LENGTH, &gt912_update_msg.file->f_pos);
    if (ret < 0)
    {
    ...
    {
        u16 temp;
        ret = gt912_update_msg.file->f_op->read(gt912_update_msg.file, (char*)buf, 2, &gt912_update_msg.file->f_pos);
        if (ret < 0)
        {
...
static u8 gup_load_section_file(u8 *buf, u32 offset, u16 length, u8 set_or_end)
{
    ...
    if (got_file_flag == HEADER_FW_READY)
    {
        ...
        {
            memcpy(buf, &gtp_default_FW[gt912_update_msg.fw_total_len + FW_HEAD_LENGTH - offset], length);
        }
        return SUCCESS;
    }
#endif
    {
        ...
        if ( (gt912_update_msg.file == NULL) || IS_ERR(gt912_update_msg.file))
        {
        ...
        if(SEEK_SET == set_or_end)
        {
            gt912_update_msg.file->f_pos = FW_HEAD_LENGTH + offset;
        }
        else    //seek end
        {
            gt912_update_msg.file->f_pos = gt912_update_msg.fw_total_len + FW_HEAD_LENGTH - offset;
        }

        ret = gt912_update_msg.file->f_op->read(gt912_update_msg.file, (char *)buf, length, &gt912_update_msg.file->f_pos);
        if (ret < 0)
        {
...
static u8 gup_burn_dsp_isp(struct i2c_client *client)
{
    ...
    gt912_update_msg.fw_burned_len += FW_DSP_ISP_LENGTH;
    GTP_DEBUG("[burn_dsp_isp]Burned length:%d", gt912_update_msg.fw_burned_len);
    ret = SUCCESS;
...
//    }
    ...
    gt912_update_msg.fw_burned_len += (FW_SECTION_LENGTH*4);
    GTP_DEBUG("[burn_fw_ss51]Burned length:%d", gt912_update_msg.fw_burned_len);
    ret = SUCCESS;
...
static u8 gup_burn_fw_dsp(struct i2c_client *client)
{
    ...
    gt912_update_msg.fw_burned_len += FW_DSP_LENGTH;
    GTP_DEBUG("[burn_fw_dsp]Burned length:%d", gt912_update_msg.fw_burned_len);
    ret = SUCCESS;
...
static u8 gup_burn_fw_boot(struct i2c_client *client)
{
    ...
    gt912_update_msg.fw_burned_len += FW_BOOT_LENGTH;
    GTP_DEBUG("[burn_fw_boot]Burned length:%d", gt912_update_msg.fw_burned_len);
    ret = SUCCESS;
...
static u8 gup_burn_fw_boot_isp(struct i2c_client *client)
{
    ...
    if(gt912_update_msg.fw_burned_len >= gt912_update_msg.fw_total_len)
    {
...
//step1:Alloc memory
    ...
    ret = gup_load_section_file(fw_boot_isp, (gt912_update_msg.fw_burned_len - FW_DSP_ISP_LENGTH), FW_BOOT_ISP_LENGTH, SEEK_SET);
    if(FAIL == ret)
    {
    ...
    gt912_update_msg.fw_burned_len += FW_BOOT_ISP_LENGTH;
    GTP_DEBUG("[burn_fw_boot_isp]Burned length:%d", gt912_update_msg.fw_burned_len);
    ret = SUCCESS;
...
static u8 gup_burn_fw_link(struct i2c_client *client)
{
    ...
    offset = gt912_update_msg.fw_burned_len - FW_DSP_ISP_LENGTH;
    ...
    gt912_update_msg.fw_burned_len += FW_GLINK_LENGTH;
    GTP_DEBUG("[burn_fw_link]Burned length:%d", gt912_update_msg.fw_burned_len);
    ret = SUCCESS;
...
static u8 gup_burn_fw_gwake(struct i2c_client *client)
{
    ...
    //start_index = gt912_update_msg.fw_burned_len - FW_DSP_ISP_LENGTH;
    ...
    //gt912_update_msg.fw_burned_len += FW_GWAKE_LENGTH;
    GTP_DEBUG("[burn_fw_gwake]Burned length:%d", gt912_update_msg.fw_burned_len);
    ret = SUCCESS;
...
s32 gup_912_update_proc(void *dir)
{
    s32 ret = 0;
    s32 update_ret = FAIL;
    u8  retry = 0;
    ...
    if (searching_file)
    {
        ...
        while ((gt912_show_len != 200) && (gt912_show_len != 100) && (timeout++ < 100))     // wait for auto update quitted completely
        {
            msleep(100);
        }
    }
    ...
    gt912_update_msg.file = NULL;
    ret = gup_check_update_file(gtp_912_i2c_connect_client, &fw_head, (u8*)dir);     //20121211
    if(FAIL == ret)
    {
    ...
    ret = gup_get_ic_fw_msg(gtp_912_i2c_connect_client);
    if(FAIL == ret)
    {
...
#endif
    ret = gup_912_enter_update_mode(gtp_912_i2c_connect_client);
    if(FAIL == ret)
    {
    ...
    while(retry++ < 5)
    {
        gt912_show_len = 10;
        gt912_total_len = 100;
        gt912_update_msg.fw_burned_len = 0;
        ret = gup_burn_dsp_isp(gtp_912_i2c_connect_client);
        if(FAIL == ret)
        {
    ...
    gt912_show_len = 20;
    ret = gup_burn_fw_gwake(gtp_912_i2c_connect_client);
        if (FAIL == ret)
        {
        ...
        gt912_show_len = 30;
        ret = gup_burn_fw_ss51(gtp_912_i2c_connect_client);
        if(FAIL == ret)
        {
        ...
        gt912_show_len = 40;
        ret = gup_burn_fw_dsp(gtp_912_i2c_connect_client);
        if(FAIL == ret)
        {
        ...
        gt912_show_len = 50;
        ret = gup_burn_fw_boot(gtp_912_i2c_connect_client);
        if(FAIL == ret)
        {
        ...
        gt912_show_len = 60;
        ret = gup_burn_fw_boot_isp(gtp_912_i2c_connect_client);
        if (FAIL == ret)
        {
        ...
        gt912_show_len = 70;
        ret = gup_burn_fw_link(gtp_912_i2c_connect_client);
        if (FAIL == ret)
        {
        ...
        gt912_show_len = 80;
        ret = gup_burn_fw_finish(gtp_912_i2c_connect_client);
        if (FAIL == ret)
        {
...
file_fail:
	if (gt912_update_msg.file && !IS_ERR(gt912_update_msg.file))
	{
        if (gt912_update_msg.old_fs)
        {
            set_fs(gt912_update_msg.old_fs);
        }
		filp_close(gt912_update_msg.file, NULL);
	}
...
u8 gup_init_update_proc(struct goodix_ts_data *ts)
{
    ...
    if (CHIP_TYPE_GT9F == ts->chip_type)
    {
        thread = kthread_run(gup_912_update_proc, "update", "fl update");
    }
    else
#endif
    {
        thread = kthread_run(gup_912_update_proc, (void*)NULL, "guitar_update");
    }
    if (IS_ERR(thread))
    {
...
{
    u8 buf;
    i2c_read_bytes(gtp_912_i2c_connect_client, addr, &buf, 1);

    buf = (buf & (~((u8)1 << bit))) | ((u8)val << bit);

    i2c_write_bytes(gtp_912_i2c_connect_client, addr, &buf, 1);
}
...
static void gup_clk_count_init(s32 bCh, s32 bCNT)
{
    ...
    buf = 0;
    i2c_write_bytes(gtp_912_i2c_connect_client, _bRW_MISCTL__MEA_MODE, &buf, 1);
    //_bRW_MISCTL__MEA_SRCSEL = 8 + bCh; //From GIO1
    buf = 8 + bCh;
    i2c_write_bytes(gtp_912_i2c_connect_client, _bRW_MISCTL__MEA_SRCSEL, &buf, 1);
    //_wRW_MISCTL__MEA_MAX_NUM = bCNT; //Set the Measure Counts = 1
    buf = bCNT;
    i2c_write_bytes(gtp_912_i2c_connect_client, _wRW_MISCTL__MEA_MAX_NUM, &buf, 1);
...
static u32 gup_clk_count_get(void)
{
    ...
    {
        i2c_read_bytes(gtp_912_i2c_connect_client, _bRO_MISCTL__MEA_RDY, buf, 1);
        ready = buf[0];
    }
    ...
    gup_bit_write(_fRW_MISCTL__MEA, 0, 0);
    i2c_read_bytes(gtp_912_i2c_connect_client, _dRO_MISCTL__MEA_VAL, buf, 4);
...
u8 gup_clk_dac_setting(int dac)
{
    ...
    i2c_read_bytes(gtp_912_i2c_connect_client, _wRW_MISCTL__RG_DMY, &buf1, 1);
    i2c_read_bytes(gtp_912_i2c_connect_client, _bRW_MISCTL__RG_OSC_CALIB, &buf2, 1);

    buf1 = (buf1 & 0xFFCF) | ((dac & 0x03) << 4);
    buf2 = (dac >> 2) & 0x3f;

    i2c_write_bytes(gtp_912_i2c_connect_client, _wRW_MISCTL__RG_DMY, &buf1, 1);
    i2c_write_bytes(gtp_912_i2c_connect_client, _bRW_MISCTL__RG_OSC_CALIB, &buf2, 1);
...
static void gup_sys_clk_init(void)
{
    ...
    buf = 0;
    i2c_write_bytes(gtp_912_i2c_connect_client, _bRW_MISCTL__RG_LDO_A18_PWD, &buf, 1);
    //_bRW_MISCTL__RG_BG_PWD = 0; //DrvMISCTL_BG_PowerON
    buf = 0;
    i2c_write_bytes(gtp_912_i2c_connect_client, _bRW_MISCTL__RG_BG_PWD, &buf, 1);
    //_bRW_MISCTL__RG_CLKGEN_PWD = 0; //DrvMISCTL_CLKGEN_PowerON
    buf = 0;
    i2c_write_bytes(gtp_912_i2c_connect_client, _bRW_MISCTL__RG_CLKGEN_PWD, &buf, 1);
    ...
    buf = 1;
    i2c_write_bytes(gtp_912_i2c_connect_client, _bRW_MISCTL__OSC_CK_SEL, &buf, 1);
}
...
s32 gup_clk_calibration(void)
{
    ...
    i2c_write_bytes(gtp_912_i2c_connect_client, _rRW_MISCTL__SWRST_B0_, &buf, 1);
    ...
    //-- ouput clk to GPIO 4
    buf = 0x00;
    i2c_write_bytes(gtp_912_i2c_connect_client, 0x41FA, &buf, 1);
    buf = 0x00;
    i2c_write_bytes(gtp_912_i2c_connect_client, 0x4104, &buf, 1);
    buf = 0x00;
    i2c_write_bytes(gtp_912_i2c_connect_client, 0x4105, &buf, 1);
    buf = 0x00;
    i2c_write_bytes(gtp_912_i2c_connect_client, 0x4106, &buf, 1);
    buf = 0x01;
    i2c_write_bytes(gtp_912_i2c_connect_client, 0x4107, &buf, 1);
    buf = 0x06;
    i2c_write_bytes(gtp_912_i2c_connect_client, 0x41F8, &buf, 1);
    buf = 0x02;
    i2c_write_bytes(gtp_912_i2c_connect_client, 0x41F9, &buf, 1);
#endif
...
static s32 gup_prepare_fl_fw(char *path, st_fw_head *fw_head)
{
    ...
    if (!memcmp(path, "update", 6))
    {
        ...
        for (i = 0; i < GUP_SEARCH_FILE_TIMES; ++i)
        {
            ...
            gt912_update_msg.file = filp_open(FL_UPDATE_PATH, O_RDONLY, 0);
            if (IS_ERR(gt912_update_msg.file))
            {
                gt912_update_msg.file = filp_open(FL_UPDATE_PATH_SD, O_RDONLY, 0);
                if (IS_ERR(gt912_update_msg.file))
                {
                    msleep(3000);
                    continue;
        ...
        else
        {
            ...
            _CLOSE_FILE(gt912_update_msg.file);
        }
    ...
    gt912_update_msg.force_update = 0xBE;     // GT9XXF ignore the 0xBE
    if (gt912_update_msg.fw_total_len != sizeof(gtp_default_FW_fl))
    {
        GTP_ERROR("Inconsistent fw size. default size: %d(%dK), file size: %d(%dK)", sizeof(gtp_default_FW_fl), sizeof(gtp_default_FW_fl)/1024, gt912_update_msg.fw_total_len, gt912_update_msg.fw_total_len/1024);
        set_fs(gt912_update_msg.old_fs);
        _CLOSE_FILE(gt912_update_msg.file);
        return FAIL;
    }
    ...
    gt912_update_msg.fw_total_len -= FW_HEAD_LENGTH;
    GTP_DEBUG("Fimrware size: %d(%dK)", gt912_update_msg.fw_total_len, gt912_update_msg.fw_total_len / 1024);

    gt912_update_msg.file->f_op->llseek(gt912_update_msg.file, 0, SEEK_SET);
    ret = gt912_update_msg.file->f_op->read(gt912_update_msg.file, (char*)gtp_default_FW_fl,
                             gt912_update_msg.fw_total_len + FW_HEAD_LENGTH,
                                &gt912_update_msg.file->f_pos);
	gt912_update_msg.fw_total_len  += FW_HEAD_LENGTH;
    set_fs(gt912_update_msg.old_fs);
    _CLOSE_FILE(gt912_update_msg.file);
...
static u8 gup_check_update_file_fl(struct i2c_client *client, st_fw_head* fw_head, char* path)
{
	...
	{
		gt912_update_msg.fw_total_len = sizeof(gtp_default_FW_fl);
	}
    ...
    //check firmware legality
    fw_checksum = 0;
    for(i = FW_HEAD_LENGTH; i < gt912_update_msg.fw_total_len; i += 2)
    {
...
s32 gup_fw_download_proc(void *dir, u8 dwn_mode)
{
    ...
    ret = gup_check_update_file_fl(gtp_912_i2c_connect_client, &fw_head, (char *)dir);
    gt912_show_len = 10;
    ...
    ret = gup_enter_update_mode_fl(gtp_912_i2c_connect_client);
    gt912_show_len = 20;
    if (FAIL == ret)
    {
    ...
    while (retry++ < 5)
    {
        ret = gup_download_fw_ss51(gtp_912_i2c_connect_client, dwn_mode);
        gt912_show_len = 60;
        if (FAIL == ret)
        {
        ...
        ret = gup_download_fw_dsp(gtp_912_i2c_connect_client, dwn_mode);
        gt912_show_len = 80;
        if (FAIL == ret)
        {
...
static u8 __gup_download_fw_dsp(struct i2c_client *client, u8 dwn_mode, int len)
{
    s32 ret = 0;

    if(GTP_FL_FW_BURN == dwn_mode)
        GTP_DEBUG("[download_fw_dsp]Begin download dsp fw---->>");
    else
        GTP_DEBUG("[download_fw_dsp]Begin check dsp fw---->>");
    //step1:select bank2
    GTP_DEBUG("[download_fw_dsp]step1:select bank2");
    ret = gup_set_ic_msg(client, _bRW_MISCTL__SRAM_BANK, 0x02);
    ...
    ret = i2c_write_bytes(client, 0xC000,
            &gtp_default_FW_fl[FW_HEAD_LENGTH + 2 * FW_DOWNLOAD_LENGTH], len);
    if (ret == -1) {
        GTP_ERROR("[download_fw_dsp]download FW dsp fail.");
        ret = FAIL;
        goto exit_download_fw_dsp;
...
int gup_reload_fw_dsp(void *dir, u8 dwn_mode)
{
	s32 ret = 0;
	u8	retry = 0;
	u8 value = 0;
	...
	ret = gup_enter_update_mode_fl(ts->client);
	if (FAIL == ret) {
		GTP_ERROR("[reload_fw_dsp]enter update mode fail.");
		goto download_fail;
	}

	while (retry++ < 5) {
		ret = __gup_download_fw_dsp(ts->client, dwn_mode, FW_DSP_LENGTH / 2); //reload 2k dsp code
		if (FAIL == ret){
			GTP_ERROR("[reload_fw_dsp]burn dsp firmware fail.");
			continue;
		}
		GTP_DEBUG("[reload_fw_dsp]UPDATE SUCCESS.");
		retry = 0;
		break;
	}

	if (retry >= 5) {
		GTP_ERROR("[reload_fw_dsp]retry timeout,RELOAD FW DSP FAIL.");
		goto download_fail;
	}
	...
	ret = i2c_write_bytes(ts->client, 0x801F, &value, 1); // clear gesture mode reg
