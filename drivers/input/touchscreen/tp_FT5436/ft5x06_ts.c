...
#if defined CONFIG_TCT_8909_POP3
#define TCT_KEY_BACK_POS_X  80
#define TCT_KEY_BACK_POS_Y  1003

#define TCT_KEY_HOME_POS_X  240
#define TCT_KEY_HOME_POS_Y  1003

#define TCT_KEY_MENU_POS_X  400
#define TCT_KEY_MENU_POS_Y  1003
#endif
...
static irqreturn_t ft5x06_ts_interrupt(int irq, void *dev_id)
{
	...
	for (i = 0; i < data->pdata->num_max_touches; i++) {
		...
		if (status == FT_TOUCH_DOWN || status == FT_TOUCH_CONTACT) {
			if(y >= TCT_KEY_BACK_POS_Y)
			{
				switch(x)
				{
				case TCT_KEY_BACK_POS_X:
					input_report_key(ip_dev, TCT_KEY_BACK, 1);
					break;
				case TCT_KEY_HOME_POS_X:
					input_report_key(ip_dev, TCT_KEY_HOME, 1);
					break;
				case TCT_KEY_MENU_POS_X:
...
static int ft5x06_auto_cal(struct i2c_client *client)
{
	...
	msleep(data->pdata->soft_rst_dly);
//wxc
	dev_info(&client->dev, "calibration ok.\n");
			...
			const u8 *data, u32 data_len)
{
	...
	for(i = 0;i < 15;i++)
	{
		...
		dev_info(&client->dev, "1~~~~1 r_buf[0] =%x  r_buf[1] = %x\n ",r_buf[0],r_buf[1]);
	...
	dev_info(&client->dev, "data_len = %d \n",data_len);
	ft5x06_i2c_write(client, w_buf, 4);


/*********Step 5:write firmware(FW) to ctpm flash*********/
	fw_ecc = 0;
	dev_info(&client->dev, "Step 5:write firmware(FW) to ctpm flash\n");
	temp = 0;
	pkt_num = (data_len) / FT_FW_PKT_LEN;
	pkt_buf[0] = 0xbf;
	pkt_buf[1] = 0x00;
    dev_info(&client->dev, "@@@@@pkt_num = %x \n",pkt_num);
	for (j = 0; j < pkt_num; j++) {
	...
	if ((data_len) % FT_FW_PKT_LEN > 0) {
		...
		for(i = 0;i < 30;i++)
		{
			...
			r_buf[0] = r_buf[1] = 0x00;
			ft5x06_i2c_read(client, w_buf, 1, r_buf, 2);
//			dev_info(&client->dev, "@~~~~@ r_buf[0] =%x  r_buf[1] = %x  j =%x\n ",r_buf[0],r_buf[1],j);
			dev_info(&client->dev, "@~~~~@ r_buf[0] =%x  r_buf[1] = %x	j =%x i =%d\n ",r_buf[0],r_buf[1],j, i);
	...
	dev_info(&client->dev, "Step 6: read out checksum\n");
	w_buf[0] = 0x64;
	ft5x06_i2c_write(client, w_buf, 1);
	msleep(300);
	...
	for(i = 0;i < 100;i++)
	{
		...
		r_buf[0] = r_buf[1] = 0x00;
		ft5x06_i2c_read(client, w_buf, 1, r_buf, 2);
		dev_info(&client->dev, "~~~~ r_buf[0] =%x  r_buf[1] = %x \n ",r_buf[0],r_buf[1]);
	...
	dev_info(&client->dev,KERN_WARNING "checksum %x %x \n",r_buf[0],fw_ecc);
	/* reset */
	w_buf[0] = FT_REG_RESET_FW;
	ft5x06_i2c_write(client, w_buf, 1);
...
//[FEATURE] Add by TCT-NB.YQJ 12/04/2014 PR-858209 tp PROXIMITY SENSOR
#ifdef CONFIG_TOUCHPANEL_PROXIMITY_SENSOR
	err = virtual_psensor_input_register(client);
	if(err < 0)
	{
		pr_err("[***%s: failed\n", __func__);
		return err;
