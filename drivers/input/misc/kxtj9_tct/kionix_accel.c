...
/*[PLATFORM]-Begin Add by TCTNB.ZXZ, PR-1040633, 2015/07/09,add for new version CTS pass*/
	ktime_t timestamp;
	timestamp = ktime_get_boottime();
/*[PLATFORM]-End Add by TCTNB.ZXZ*/
		...
		if (atomic_read(&acceld->accel_enable_resume) > 0) {
			...
			if (err < 0) {
				...
				/*[PLATFORM]-Begin Add by TCTNB.ZXZ, PR-1040633, 2015/07/09,add for new version CTS pass*/
					...
					input_event(acceld->input_dev, EV_SYN,
							SYN_TIME_NSEC,
		...
		if (atomic_read(&acceld->accel_enable_resume) > 0) {
			...
			if (err < 0) {
				...
				acceld->accel_data[acceld->axis_map_z] =
				    (acceld->negate_z ? -z : z) +
				    acceld->accel_cali[acceld->axis_map_z];
//[PLATFORM]-Begin Add by TCTNB.ZXZ, PR-987271, 2015/04/30, the ACCEL_CONVERT in HAL had been changed to ((GRAVITY_EARTH) / 1024),so here we needn't shift the rawdata
#if 0
//[PLATFORM]-Add by TCTNB.ZXZ, RR-966458, 2015/04/02, add KXTJ2-1009 Gsensor  for HAL 16 bit
acceld->accel_data[acceld->axis_map_x] =acceld->accel_data[acceld->axis_map_x] << 4;
acceld->accel_data[acceld->axis_map_y] =acceld->accel_data[acceld->axis_map_y]<< 4;
acceld->accel_data[acceld->axis_map_z] =acceld->accel_data[acceld->axis_map_z]<< 4;
#endif
//[PLATFORM]-End Add by TCTNB.ZXZ
				if (atomic_read(&acceld->accel_input_event)
					> 0) {
