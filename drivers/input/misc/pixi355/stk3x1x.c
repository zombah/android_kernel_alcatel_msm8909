...
//modify START by yijun.chen@tcl.com to improve als performance(HW)-pr1065727-20150815
#define STK_IRC_MIN_IR_CODE		5	//50
#define STK_IRC_ALS_DENOMI		100
#define STK_IRC_ALS_NUMERA		200//	92
...
//modify END by yijun.chen@tcl.com to improve als performance(HW)-pr1065727-20150815
#ifdef STK_IRS
...
static int32_t stk3x1x_enable_ps(struct stk3x1x_data *ps_data, uint8_t enable, uint8_t validate_reg)
{
    ...
    if(enable)
	{
		...
		if(!validate_reg)		
		{
			...
			input_event(ps_data->ps_input_dev,
				EV_SYN, SYN_TIME_SEC,
...
#endif /* #ifdef STK_CHK_REG */
		{
			ktime_t timestamp;/*Cedar.Tan add to be consistent with sensor hal CLOCK_BOOTTIME, PR1020427, 20150713 */
			usleep_range(4000, 5000);
			ret = stk3x1x_get_flag(ps_data);
			if (ret < 0)
				return ret;
			timestamp = ktime_get_boottime();/*Cedar.Tan add to be consistent with sensor hal CLOCK_BOOTTIME, PR1020427, 20150713 */
...
static int32_t stk3x1x_enable_als(struct stk3x1x_data *ps_data, uint8_t enable)
{
    ...
    if (enable)
    {
		ps_data->als_lux_last = 0xffff;/* Cedar.Tan, PR1053070, 20150728 */
...
static ssize_t stk_als_lux_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
	...
	/* + Cedar.Tan add to be consistent with sensor hal CLOCK_BOOTTIME, PR1020427, 20150713 */
	input_event(ps_data->als_input_dev,
		EV_SYN, SYN_TIME_SEC,
		ktime_to_timespec(timestamp).tv_sec);
	input_event(ps_data->als_input_dev,
		EV_SYN, SYN_TIME_NSEC,
...
static ssize_t stk_ges_code_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
	...
	/* + Cedar.Tan add to be consistent with sensor hal CLOCK_BOOTTIME, PR1020427, 20150713 */
	input_event(ps_data->ges_input_dev,
		EV_SYN, SYN_TIME_SEC,
		ktime_to_timespec(timestamp).tv_sec);
	input_event(ps_data->ges_input_dev,
		EV_SYN, SYN_TIME_NSEC,
...
static int stk_get_offset(struct stk3x1x_data *ps_data, uint16_t ct_value)
{
	if(ct_value <= 100)
	{
		ps_data->stk_max_min_diff = 200;
		ps_data->stk_ht_n_ct = 110;
		ps_data->stk_lt_n_ct = 50;
	}
	else if(ct_value <= 200)
	{
		ps_data->stk_max_min_diff = 200;
		ps_data->stk_ht_n_ct = 130;
		ps_data->stk_lt_n_ct = 70;
	}
	else if(ct_value <= 400)
	{
		ps_data->stk_max_min_diff = 200;
        ...
        {
                ps_data->stk_max_min_diff = 200;
                ps_data->stk_ht_n_ct = 150;
                ps_data->stk_lt_n_ct = 90;
        }
	else
	{
		ps_data->stk_max_min_diff = 200;
...
static void stk_als_poll_work_func(struct work_struct *work)
{
	...
	int32_t reading, reading_lux, als_comperator, flag_reg, als_comperator_led;
	#ifdef STK_IRS	
	int ret;
	#endif
	ktime_t timestamp;/*Cedar.Tan add to be consistent with sensor hal CLOCK_BOOTTIME, PR1020427, 20150713 */
	...
	if(ps_data->ir_code)
	{
		ps_data->als_correct_factor = 3090;	//1000;
...
//modify START by yijun.chen@tcl.com to improve als performance(HW)-pr1065727-20150815
			als_comperator_led = reading * STK_IRC_ALS_NUMERA_LED / STK_IRC_ALS_DENOMI_LED;
			if(ps_data->ir_code > als_comperator)
				ps_data->als_correct_factor = STK_IRC_ALS_CORREC;
			else if(ps_data->ir_code < als_comperator_led)
				ps_data->als_correct_factor = STK_IRC_ALS_CORREC_LED;
//modify END by yijun.chen@tcl.com to improve als performance(HW)-pr1065727-20150815
		}
#ifdef STK_DEBUG_PRINTF				
...
//delete START by yijun.chen@tcl.com to improve als performance(HW)-pr1065727-20150815
/*
#ifdef STK_IRS
...
#endif
*/
//delete END by yijun.chen@tcl.com to improve als performance(HW)-pr1065727-20150815
