 ...
 * ------------------------------------------------------------------------------------
* [PLATFORM]-Add- by TCTNB.XQJ, RR-	876676, 2015/01/13, add for pop3 led 
 */
...
//#define CONFIG_TCT_8X16_IDOL347
...
struct sn31xx_led {
    ...
    #if defined(CONFIG_TCT_8X16_IDOL3) || defined(CONFIG_TCT_8X16_IDOL347) || defined(CONFIG_TCT_8909_PIXI35) || defined(CONFIG_TCT_8909_PIXI355) || defined(CONFIG_TCT_8909_PIXI355_TMO)
...
{               
    ...
    if(((value == LED_OFF) && (led->state == SN31XX_OFF))
        || ((value != LED_OFF) && (led->state == SN31XX_ON)))
    {
...
#else
        // Pixi3-5 nend 7mA current
		sn31xx_write_byte(led->client,0x03,0x04); //i max 10ma 
		sn31xx_write_byte(led->client,0x04, 0xb3); // avg 7ma
#endif
...
{                               
    ...
    if(((bblink == 0) && (led->state == SN31XX_OFF))
        || ((bblink == 1) && (led->state == SN31XX_BLINK1))
        || ((bblink == 2) && (led->state == SN31XX_BLINK2)))
    {
        //pr_err(" %s same value, don't change.\n", __func__);
        return ;
    }
...
#if defined(CONFIG_TCT_8909_PIXI355_TMO) || defined(CONFIG_TCT_8909_PIXI355_TF)
        sn31xx_write_byte(led->client,0x0a, 0x00); //T0 0S
        sn31xx_write_byte(led->client,0x10, 0x28);//T1 0.26s,Tt2 1.04s
        sn31xx_write_byte(led->client,0x16, 0x2e);//T3 0.26s,T4 8.36s
        sn31xx_write_byte(led->client,0x1C, 0x00);//UPDATE
#else
        sn31xx_write_byte(led->client,0x0a, 0x00); //T0 0S
        sn31xx_write_byte(led->client,0x10, 0x80);//T1 2s,Tt2 0s
        sn31xx_write_byte(led->client,0x16, 0x88);//T3 2s,T4 1s
		sn31xx_write_byte(led->client,0x1C, 0x00);//UPDATE
#endif
        led->state = SN31XX_BLINK1;
    #if defined(CONFIG_TCT_8X16_IDOL3) || defined(CONFIG_TCT_8X16_IDOL347) || defined(CONFIG_TCT_8909_PIXI35) || defined(CONFIG_TCT_8909_PIXI355) || defined(CONFIG_TCT_8909_PIXI355_TMO)
        gpio_direction_output(led->en_gpio2, 1);/*[BUGFIX]-Add by TCTNB.ZXZ,PR-893177 2015/01/07,add  led ic sn31xx,gpio2 is used for ind_led*/
    #endif        
           ...
           //msleep(100);
           sn31xx_write_byte(led->client,0x2f, 0x00); //reset
           sn31xx_write_byte(led->client,0x00, 0x01); //
           sn31xx_write_byte(led->client,0x02, 0x00); // 
#if defined CONFIG_TCT_8X16_IDOL347
	    sn31xx_write_byte(led->client,0x03,0x08); //i max 5ma 
	    sn31xx_write_byte(led->client,0x04, 0x0f); 
...
#else
		sn31xx_write_byte(led->client,0x0a, 0x00); //T0 0S
		sn31xx_write_byte(led->client,0x10, 0x60);//T1 1s,Tt2 0s
		sn31xx_write_byte(led->client,0x16, 0x66);//T3 1s,T4 0.5s
		sn31xx_write_byte(led->client,0x1C, 0x00);//UPDATE
#endif
		led->state = SN31XX_BLINK2;
...
/*[BUGFIX]-End by TCTNB.ZXZ*/
    ...
    #if defined(CONFIG_TCT_8X16_IDOL3) || defined(CONFIG_TCT_8X16_IDOL347) || defined(CONFIG_TCT_8909_PIXI35) || defined(CONFIG_TCT_8909_PIXI355) || defined(CONFIG_TCT_8909_PIXI355_TMO)
    gpio_direction_output(led->en_gpio2, 0);/*[BUGFIX]-Add by TCTNB.ZXZ,PR-893177 2015/01/07,add  led ic sn31xx,gpio2 is used for ind_led*/
    #endif
    led->state = SN31XX_UNKNOWN;
...
#if (!defined CONFIG_TCT_8X16_IDOL3 )&&(!defined CONFIG_TCT_8X16_IDOL347)&&(!defined(CONFIG_TCT_8909_PIXI35))/*[BUGFIX]-Mod by TCTNB.XQJ,PR-817507 2014/10/23,not need in idol3*/
...
static void sn31xx_shutdown(struct i2c_client *client)
{
    struct sn31xx_led *led = i2c_get_clientdata(client);

    pr_err(" sn31xx_shutdown gpio=%d\n",led->en_gpio);
    gpio_direction_output(led->en_gpio, 0);/*[BUGFIX]-Add by TCTNB.XQJ,PR-817507 2014/10/23,add  led ic sn31xx,gpio is used for led ic*/
#if defined(CONFIG_TCT_8X16_IDOL3) || defined(CONFIG_TCT_8X16_IDOL347) || defined(CONFIG_TCT_8909_PIXI35) || defined(CONFIG_TCT_8909_PIXI355) || defined(CONFIG_TCT_8909_PIXI355_TMO)
    gpio_direction_output(led->en_gpio2, 0);/*[BUGFIX]-Add by TCTNB.ZXZ,PR-893177 2015/01/07,add  led ic sn31xx,gpio2 is used for ind_led*/
#endif
